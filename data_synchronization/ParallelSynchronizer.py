...
class ParallelSynchronizer:
    ...
    def sync_with_movement(self, csv_full_path, wisci_file, log=None, output_path_manual=None):
        ...
        if video_duration > 15: # process videos that are at least 15s long
            ...
            sync_failed = 1
            with self.lock:
                log.update_log(original_mp4_basename, 'mp4_length', video_duration)
                log.update_log(original_mp4_basename, 'sync_error_msg', "Video too short (under 15s).")

        if log:
            with self.lock:
                log.update_log(original_mp4_basename, 'mp4_length', video_duration)
                log.update_log(original_mp4_basename, 'frames', best_n_samples)
                log.update_log(original_mp4_basename, 'path_wisci', wisci_file)
                log.update_log(original_mp4_basename, 'corr', best_corr)
                log.update_log(original_mp4_basename, 'lag', best_lag)
    ...
    def sync_mp4_folder(self):
        ...
        for path_csv_basename in movement_csv_files_basenames:
            ...
            log = SyncLogger(self.log_table_path)
            try:
                log.process_new_file(original_mp4_basename)
                to_be_processed.append(path_csv_basename)
                logs.append(log)
        ...
        with multiprocessing.Manager() as manager:
            ...
            with multiprocessing.Pool() as pool:
                # pool.map(self.process_file, to_be_processed)
                pool.starmap(self.process_file, zip(to_be_processed, logs))
        
    def process_file(self, path_csv_basename, log):

        original_mp4_basename = path_csv_basename[:-len(self.dlc_suffix)] + ".mp4"
        print("Processing: ", original_mp4_basename)
        wisci_file = find_corresponding_wisci(original_mp4_basename, self.wisci_server_path)
        try:
            self.sync_with_movement(os.path.join(self.dlc_csv_folder, path_csv_basename), wisci_file, log)
        except GettingAccelerometerDataFailed as e:
            with self.lock:
                log.update_log(original_mp4_basename, 'sync_error_msg', "GettingAccelerometerDataFailed")
                log.update_log(original_mp4_basename, 'sync_failed', 1)
        except Exception as e:
            with self.lock:
                log.update_log(original_mp4_basename, 'sync_error_msg', traceback.format_exc())
                log.update_log(original_mp4_basename, 'sync_failed', 1)
        finally:
            with self.lock:
                log.save_to_csv()
...
if __name__=="__main__":
    ...
    synchronizer = ParallelSynchronizer(dlc_csv_folder, wisci_server_path, log_table_path, sigma_video, sigma_wisci, sync_images_folder, visualize)
